(function( blocks, element, editor, components, i18n ) {
	...
	registerBlockType( 'photography-core/folders', {
		title: __( 'Folders (MeowApps)', 'photography-core' ),
		icon: 'camera',
		...
		edit: function( { attributes, setAttributes, isSelected } ) {
			...
			let options = mwt_block_params.folders.map(x => { return { value: x.id, label: x.padded_name } });
			options.unshift( { value: 0, label: "Main folder" } );
			let info = !attributes.folderName ?
				__('Display the folders', 'photography-core') :
				'Display the folders in "' + folderName + '"';
			let element = createElement(
				'div', { key: 'mwt-folders-div', className: 'mwt-folders' },
		...
		save: function( props ) {
			...
			const container = createElement(
				'div', { className: 'mwt-folders' }, "[mwt-folders" + pFolderId + "]"
			);
			return container;
		}
